document.addEventListener("DOMContentLoaded", function () {
    Promise.all([
        ...
        fetch("drinks.json").then(response => response.json()).catch(error => console.error("Error loading drinks.json:", error)),
        fetch("Breads.json").then(response => response.json()).catch(error => console.error("Error loading Breads.json:", error)),
        fetch("soup.json").then(response => response.json()).catch(error => console.error("Error loading soup.json:", error)),
        fetch("Rice.json").then(response => response.json()).catch(error => console.error("Error loading Rice.json:", error))
    ])
    .then(([vegData, nonvegData, drinksData, breadsData, soupData, riceData]) => {
        if (vegData) populateMenu(vegData, "vegetarian-menu");
        if (nonvegData) populateMenu(nonvegData, "nonvegetarian-menu");
        if (drinksData) populateMenu(drinksData, "drinks-menu");
        if (breadsData) populateMenu(breadsData, "breads-menu");
        if (soupData) populateMenu(soupData, "soup-menu");
        if (riceData) populateMenu(riceData, "rice-menu");
    })
